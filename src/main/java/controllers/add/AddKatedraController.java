...
public class AddKatedraController {
	...
	@FXML
	public void add() {
		if ( (nameField.getText().length() != 0) && (adresField.getText().length() != 0) && readWydzial != null ) {	
			Katedra kat = new Katedra(nameField.getText(), adresField.getText(), readWydzial.getIdWydzial());
			try {
				mainController.oos.writeObject("addkatedra");
				...
				try {
					...
					statusLabel.setText("Dodano katedrê");
				}
			} catch (IOException e) {
