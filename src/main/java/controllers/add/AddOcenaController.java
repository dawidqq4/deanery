...
public class AddOcenaController {
	...
	public void add() {
		...
		if ((ocenaToInt > 1) && (ocenaToInt < 6) && readStudent != null && readPrzedmiot != null && readWykladowca != null) {
			Ocena oc = new Ocena(ocenaToInt, readStudent.getidStudent(), readPrzedmiot.getIdPrzedmiot(), readWykladowca.getIdWykladowca());
			try {
				mainController.oos.writeObject("addocena");
				...
				try {
					...
					statusLabel.setText("Dodano ocene");
				}
			} catch (IOException e) {
